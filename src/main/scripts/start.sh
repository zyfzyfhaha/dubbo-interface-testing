...
GC_OPTS=" -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -Xloggc:$LOGS_DIR/gc.log "
JAVA_DEBUG_OPTS=""
if [ "$1" = "debug" ]; then
...
nohup /usr/java/jdk1.8.0_66/bin/java $JAVA_OPTS $GC_OPTS $DUMP_ON_OUT_OF_MEM $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -Ddubbo.container=spring,jetty -Ddubbo.log4j.level=ERROR -Ddubbo.application.logger=slf4j -Djava.ext.dirs=/usr/java/jdk1.8.0_66/jre/lib:/usr/java/jdk1.8.0_66/jre/lib/ext:$LIB_DIR:$CONF_DIR -cp $CONF_DIR:$CONF_DIR/lib:$CONF_DIR/conf/:$CONF_DIR/conf/logback.xml:$CONF_DIR/conf/spring:$CONF_DIR/conf/dubbo-consumer:$CONF_DIR/conf/dubbo-provider com.cgw360.cls.bss.util.DubboStarter >> $STDOUT_FILE 2>&1 &
